...
class ZTCP {
      ...
      this.socket.write(msg, null, (err) => {
        ...
        if (this.timeout) {
          timer = setTimeout(
            ...
            connect ? this.timeout : this.timeout
          );
        }
      });
    });
    ...
    try {
      await new Promise((resolve, reject) => {
        this.socket.write(buf, null, (err) => {
          if (err) {
            console.error(
              `[TCP][SEND_CHUNK_REQUEST] Error sending chunk request: ${err.message}`
            );
  ...
  // Retrieve fingerprint templates for a user
  ...
  // Start enrollment by device, userid and finger index
  async startEnroll(userId, fingerIndex, waitTimeout) {
    this.freeData();
    ...
    try {
      ...
      if (fingerIndex < 0 || fingerIndex > 9)
        throw new Error("Invalid finger index");

      // Compile
      const userIdStr = userId.toString();
      const userIdBuf = Buffer.from(userIdStr, "utf8");

      // Buffer
      const enrollBuffer = Buffer.alloc(26, 0);
      userIdBuf.copy(enrollBuffer, 0);
      enrollBuffer.writeUInt8(fingerIndex, 24);
      enrollBuffer.writeUInt8(1, 25);
      ...
      let enrollTimedOut = false;

      // Create a timeout for enrollment response
      const timeout = setTimeout(() => {
        (async () => {
          try {
            enrollTimedOut = true;
            console.error("Device unresponsive during enroll, likely frozen.");
            //
            await this.disableDevice();
            await this.executeCmd(COMMANDS.CMD_CANCELCAPTURE, "");
            await new Promise((res) => setTimeout(res, 1000));
            ...
            console.error("Timeout recovery failed:", timeoutErr);
          }
        })();
      }, waitTimeout);

      // Start enroll
      await this.executeCmd("0x003d", enrollBuffer);

      // Buffer
      const eventBuf = createTCPHeader(
        COMMANDS.CMD_REG_EVENT,
        this.sessionId,
        this.replyId,
        Buffer.from([0x01, 0x00, 0x00, 0x00])
      ...
      const handleData = async (data) => {
        //
        if (enrollTimedOut) return;
        //
        const eventCode = data.readUInt16LE(8);
        ...
        if (eventCode === 0x1f4 && buf.length >= 35) {
          // Clear timeout
          clearTimeout(timeout);
          const decodeResult = async (buffer) => {
            try {
              const rawFpSize = buffer.readUInt16LE(18);
              const rawUserId = buffer
                .slice(20, 29)
                .toString("utf8")
                .replace(/\0/g, "");
              const rawFingerIndex = buffer.readUInt8(buffer.length - 1);

              if (rawFpSize > 0 && rawUserId && rawFingerIndex >= 0) {
                console.log("Enroll success!");
              } else {
                console.error("Enroll failed!");
              }

              // After handling, remove the listener
              if (
                this.socket &&
                typeof this.currentEnrollHandler === "function"
              ) {
                this.socket.off("data", this.currentEnrollHandler);
                this.currentEnrollHandler = null;
              }
            } catch (e) {
              console.error("Failed to decode enroll result:", e);
          ...
          await decodeResult.call(this, buf);
        }
      };

      // Before adding, remove old listener if exists
      if (this.socket && typeof this.currentEnrollHandler === "function") {
        this.socket.off("data", this.currentEnrollHandler);
      }

      // Save and add the new handler
      this.currentEnrollHandler = handleData.bind(this);
      this.socket.on("data", this.currentEnrollHandler);

      if (!this.socket) throw new Error("Socket not connected during enroll");
      // Send the event
  ...
  // Deletes a fingerprint template for a user
  async deleteFingerprintTemplate(userId, fingerIndex) {
    ...
    const userIdBuf = Buffer.alloc(24, 0x00);
    userIdBuf.write(userId, 0, "utf8");

    const deleteBuf = Buffer.alloc(25, 0x00);
    userIdBuf.copy(deleteBuf, 0);
    deleteBuf.writeUInt8(fingerIndex, 24);

    await this.disableDevice();

    await this.executeCmd(COMMANDS.CMD_DEL_FPTMP, deleteBuf);

    await this.executeCmd(COMMANDS.CMD_REFRESHDATA, Buffer.alloc(0));

    await this.enableDevice();

    console.log(`Finger index ${fingerIndex} deleted for user ${userId}`);
  }
}
